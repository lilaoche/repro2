...
def GetPunch():
    ...
    if identity == '小组长':
        if len(PunchModel.query.filter_by(id=token_id, day=day).all()) == 0:
            ...
            if PunchModel.query.filter_by(id=token_id,
                                          day=day).first().identity != identity or PunchModel.query.filter_by(
                    id=token_id, day=day).first().department != department:
...
def SelectPunch():
    ...
    for i in token_list[1:]:
        role += chr(int(i))
    sql_list = [
        'select id,name,day,go_time,leave_time,department,identity,intervals,grp_agree,htp_agree from punch_data']
    if role != '老板':
        ...
        sql_list.append(
            " where (identity = '员工' or identity = '小组长' or identity = '主管') and grp_agree = 'true' and htp_agree = 'true'")
    # 对值为''的键值对过滤
    ...
    if len(PunchQuery) == 3:
        ...
        if 'intervals' in list(PunchQuery.keys()):
            sql_list.append(" and str_to_date(intervals, '%H:%i:%s') > str_to_date('{}', '%H:%i:%s')".format(
                ''.join([PunchQuery['intervals'], ':00'])))
